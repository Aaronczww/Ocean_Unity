    ...
    SubShader
    {
        Pass
        {
            ...
            float4 frag(FFTVertexOutput i):SV_TARGET
            {
                ...
                float2 dDdy = -0.5 * (tex2D(_Displacement,i.texcoord + float4(0,-texelSize,0,0)).rb - tex2D(_Displacement,i.texcoord + float4(0,texelSize,0,0)).rb);
                float2 dDdx = -0.5 * (tex2D(_Displacement,i.texcoord + float4(-texelSize,0,0,0)).rb - tex2D(_Displacement,i.texcoord + float4(texelSize,0,0,0)).rb);
